# College Search — New York City (20 mile radius + all of Long Island)

XC/Track + Computer Science. Last updated 2026-08-22 (v3 — radius tightened, CUNY and Long Island filled in)

> **The ring changed and so did most of this document.** It is now **20 driving miles from Midtown
> plus all of Nassau and Suffolk County** regardless of distance — a shape, not a circle, because
> Long Island is a commutable corridor while Westchester and central New Jersey at the same
> distance are a different decision. That cost **ten schools, including target-tier Fairfield and
> deep-tier Monmouth**. Filling in the CUNY system and Long Island added **sixteen**. Three schools
> turned out to have no men's cross country program at all. Net: 34 → **37**. See §2.

Live version, with per-school pages, per-race results and meet maps:
<https://timhibbard.github.io/xc-cs-college-board/new-york.html>

## 1. Athlete profile

| Event | Current | Projected |
|---|---|---|
| 1600m | 4:21 | ~4:18 |
| 3200m | 9:40 | 9:29 |
| 5K XC | 16:29 | ~15:59 |

Converted: **1500 4:03** · **3000 8:53** · **track 5000 15:20** · **8K XC 26:07** · **10K XC 32:54**

Target: a program where he would arrive as their **5th through 9th man**. Being ahead of a team's
#1 is not a good sign — it means the program is thin and he would train alone. The track 5000 is
anchored on the 3200 rather than the 1600 on purpose; the derivation is in `../methodology.html`
§1a. The framing rule and the practical notes are in `greenville-sc.md` §1 and §9 and apply
identically here.

## 2. Read this first: the radius change, the CUNY finding, and three corrections

### 2a. What the tighter ring costs

Ten schools leave the board and two of them were load-bearing.

| School | Mi | Tier it held | What was lost |
|---|---|---|---|
| **Fairfield** | 50 | **Target** | One of only three target-tier fits in the metro. **6th man at the MAAC championship** on a tight 59-second seven — a good read. Its NCAA regional 10K disagreed (the squad came apart to a 141-second spread and he would have been their #1), so it was flagged as the metro's highest-value verification. Connecticut is now entirely out of range, so the question is moot. |
| **Monmouth** | 45 | **Deep** | Unambiguous and the same shape as NYU: about 21 seconds outside their scoring seven with a 94-second spread that left room to move up. |
| **Army West Point** | 50 | Verify | Free tuition, excellent CS, a real distance program — and a congressional nomination plus a five-year active-duty obligation, so a different life decision rather than a college choice. |
| **Sacred Heart** | 52 | Caution | Was the one school plotting outside the old circle, kept as a boundary case. |
| **Mercy University** | 25 | Verify | Westchester. |
| **Pace** | 30 | Verify | Westchester. |
| **Manhattanville** | 30 | Verify | Westchester. |
| **SUNY Purchase** | 30 | Verify | Westchester — and the school whose `❌ none` CS reading turned out to be a program filed under mathematics, which is the caveat that now carries across the whole board. |
| **Ramapo College** | 25 | Verify | New Jersey. |
| **Drew University** | 30 | Verify | New Jersey. |

**Princeton and Rutgers also leave the cut list** — both were removed on level, and both are now
simply out of range.

Honest accounting: the radius change cost **one Target, one Deep, and eight unknowns**.

### 2b. The finding that changes this metro: ten CUNY colleges

Filling in the CUNY system added eight programs — **Hunter, Brooklyn, City College (CCNY), Lehman,
John Jay, York, Medgar Evers** and, at Division 2, the **College of Staten Island**. With **Baruch**
and **Queens College**, which were already here, that is **ten CUNY colleges in range, every one
sponsoring men's cross country, every one granting a bachelor's in computer science, and every one
under $6,000 in federal average net price.**

| CUNY college | Div | Mi | CS share | Net cost/yr |
|---|---|---|---|---|
| Hunter College | D3 | 2 | 9.9% | **$2,984** |
| Baruch College | D3 | 5 | 9.6% | $3,033 |
| Brooklyn College | D3 | 10 | 10.1% | $3,103 |
| Lehman College | D3 | 11 | 8.2% | $3,148 |
| John Jay | D3 | 2 | 7.4% | $3,203 |
| CCNY | D3 | 6 | 4.9% | $3,776 |
| Queens College | D2 | 10 | 10.7% | $4,195 |
| York College | D3 | 12 | 6.3% | $4,456 |
| College of Staten Island | D2 | 16 | 7.8% | $5,579 |
| Medgar Evers | D3 | 7 | 4.8% | $5,718 |

Hunter and Brooklyn each award about **10% of their bachelor's degrees in computer science** — a
higher share than Fordham, NYU or any D1 in this metro except NJIT. All ten are **completely
unmeasured on running**, which is the point: at CUNYAC level the odds he is inside their seven are
good, and it costs one email per school to find out.

**Read that net price with care.** It is an average over a student body that is overwhelmingly
in-state and living at home. A South Carolina applicant pays the out-of-state rate — about
**$15,300** at CUNY, still low — and **most of these campuses have no dormitories at all**, so
housing means renting in New York City and that cost appears nowhere in the federal file. The
correct reading: CUNY is the cheapest tuition on the page by a wide margin and the most expensive
housing problem on it.

### 2c. Three schools were on this board in error

All three carried a **Verify** tier, which is exactly how that tier fails — it reads as a program
awaiting measurement rather than a program that does not exist. All three are kept on the map
rather than deleted so the correction stays visible.

- **NYIT.** Suspended its *entire* intercollegiate athletics program in August 2020 and has not
  reinstated it. This is the one that stings: NYIT was called "the standout on paper" in the
  previous version at 23.9% CS degrees, D2, 81% admit. There is no team to verify.
- **Montclair State.** Sponsors men's indoor and outdoor track and field but **not** men's cross
  country. Still worth a look as a track-only program if that trade is acceptable.
- **William Paterson.** Sponsors neither men's cross country nor men's track and field.

### 2d. Still standing from the previous version

**Hofstra is cut, reversing this document's #2 pick.** The first version listed it as *"Coastal
Athletic D1, ~40s gap, Long Island campus with actual space to train, clean athletic and academic
fit."* That rested on a 14:32 outdoor 5000. Hofstra then **won the CAA championship with a top
seven of 23:02–24:45** and his projection lands roughly **82 seconds outside** the scoring seven.
The 14:32 was a floor set by whichever athlete happened to race it, not a measure of the squad.
Hofstra is in Hempstead, so it stays on the board under the Long Island rule — as a cut.

**Wagner sits at Verify, not Target.** Previously in the ideal band ("Staten Island, ~40s gap, easy
admission, D1. Underrated."). Their front two ran 25:55 and 26:05 and he would be their 3rd man —
but their 4th runner is at 28:59 across a **360 second** 1-through-7 spread, and only six runners
are on file. Something is wrong with either the squad or the result. Worth one email, not five.

**NYU is Deep — too good, not too thin.** They won the UAA with a seven spanning 24:50–25:32, a
**42 second spread**, and he would be about 35 seconds outside it. With Monmouth out of range it is
now the metro's **only** Deep-tier school. D3 means no scholarship is at stake in aiming high; the
9% admit rate is the real gate, and a coach's support is what moves it.

**Iona's cut is confirmed in the most emphatic way available.** At the MAAC championship all seven
Iona runners crossed **within 0.8 seconds of each other** in a deliberate pack finish, 114 seconds
ahead of his projection. When a team can do that, there is no back end to slot into.

### 2e. Net effect

**Thirty-seven schools, two of them target tier** — Fordham and Seton Hall, and Seton Hall's rating
still rests on track marks rather than cross country results. **Thirty are at Verify**, meaning
unmeasured. This is still the weakest of the three metros on running fit and now unambiguously the
strongest on cost, and those two facts point at the same list of schools.

## 3. How to read the numbers below

| Column | What it means |
|---|---|
| **XC slot** | where his projection would have finished inside their top seven, averaged over every 2025 race on file. 1 = ahead of their #1. |
| **vs their 7th** | seconds between his projection and their 7th man. `+35s` = 35 seconds *outside* the seven; `−12s` = 12 seconds *inside* it. Blank where fewer than seven of their runners finished. |
| **1–7 spread** | seconds from their #1 to their #7 — the most course-independent number here, because it compares a team only to itself. Wide = a soft back end to slot into. Tight = no room (see Iona, at 0.8 seconds). |
| **Tier** | Target = clean 4th–9th man fit · Deep = just outside the travel squad · Verify = **unmeasured**, no cross country data yet — a placeholder for a lookup nobody has done, not a judgement · Caution = he arrives at or ahead of their #1 |
| **Net cost/yr** | federal average net price after grant aid, from the College Scorecard — **not** sticker price. Out-of-state rates are used at public schools, since he is a South Carolina resident. |
| **CS** | ✅ with a percentage = share of that school's bachelor's degrees awarded in computer science, from federal completion data. `❌ none` means the federal file shows no bachelor's CS program — usually a real absence, occasionally CS filed under mathematics. |

**Thirty of the thirty-seven schools here are at Verify**, because they have no cross country
results on file at all. Those tiers are placeholders, not judgements.

## 4. Master list

Miles = approximate driving distance from Midtown Manhattan. **Anything past 20 miles is on Long
Island and in range by that rule.** School names link to a full detail page with per-race results,
a map of every meet they attended last year, cost and admissions detail.

### Division 1

| School | Location | Mi | Conf. | CS | SAT | Accept | Net cost/yr | XC slot | vs their 7th | 1–7 spread | Tier |
|---|---|---|---|---|---|---|---|---|---|---|---|
| **[Fordham](https://timhibbard.github.io/xc-cs-college-board/school.html?s=fordham)** | Bronx NY | 8 | Atlantic 10 | ✅ 4.6% | 1320–1480 | 59% | $44,338 | 6 | −12s | 95s | Target |
| **[Seton Hall](https://timhibbard.github.io/xc-cs-college-board/school.html?s=seton-hall)** | South Orange NJ | 18 | Big East | ✅ 2.4% | 1220–1390 | 73% | $31,446 | — | — | — | Target |
| **[NJIT](https://timhibbard.github.io/xc-cs-college-board/school.html?s=njit)** | Newark NJ | 12 | America East | ✅ 33.0% | 1210–1460 | 65% | $16,504 | — | — | — | Verify |
| **[Stony Brook](https://timhibbard.github.io/xc-cs-college-board/school.html?s=stony-brook)** | Stony Brook NY | 60 | CAA | ✅ 9.0% | 1330–1500 | 49% | $18,784 | — | — | — | Verify |
| **[Wagner](https://timhibbard.github.io/xc-cs-college-board/school.html?s=wagner)** | Staten Island NY | 12 | NEC | ✅ 3.8% | 1190–1290 | 88% | $28,241 | 3 | — | 360s | Verify |
| **[Fairleigh Dickinson](https://timhibbard.github.io/xc-cs-college-board/school.html?s=fairleigh-dickinson)** | Teaneck NJ | 12 | NEC | ✅ 2.0% | not reported | 91% | $15,404 | 1 | −143s | 128s | Caution |
| **[LIU](https://timhibbard.github.io/xc-cs-college-board/school.html?s=liu)** | Brooklyn NY | 5 | NEC | ✅ 3.3% | 1110–1350 | 86% | $33,062 | 1 | — | 143s | Caution |
| **[Manhattan](https://timhibbard.github.io/xc-cs-college-board/school.html?s=manhattan)** | Bronx NY | 10 | MAAC | ✅ 6.1% | 1180–1358 | 79% | $27,256 | 2 | — | 112s | Caution |
| **[Saint Peter's](https://timhibbard.github.io/xc-cs-college-board/school.html?s=saint-peter-s)** | Jersey City NJ | 5 | MAAC | ✅ 6.9% | not reported | 90% | $12,199 | 2 | −205s | 212s | Caution |

> **Stony Brook** is 60 miles out and here only by the Long Island rule, so treat the commute
> honestly. It has the strongest CS department on Long Island and a full D1 CAA program — and at D1
> CAA level the measurement is likely to be unkind. Hofstra, from the same conference, is the
> cautionary example.

### Division 2 (scholarship money exists)

| School | Location | Mi | Conf. | CS | SAT | Accept | Net cost/yr | XC slot | vs their 7th | 1–7 spread | Tier |
|---|---|---|---|---|---|---|---|---|---|---|---|
| **[Adelphi](https://timhibbard.github.io/xc-cs-college-board/school.html?s=adelphi)** | Garden City NY | 25 | NE10 | ✅ 5.8% | 1120–1340 | 66% | $30,783 | — | — | — | Verify |
| **[Caldwell](https://timhibbard.github.io/xc-cs-college-board/school.html?s=caldwell)** | Caldwell NJ | 20 | CACC | ✅ 9.3% | 1130–1370 | 71% | $24,691 | — | — | — | Verify |
| **[College of Staten Island](https://timhibbard.github.io/xc-cs-college-board/school.html?s=college-of-staten-island)** | Staten Island NY | 16 | East Coast | ✅ 7.8% | 890–1250 | 92% | $5,579 | — | — | — | Verify |
| **[Felician](https://timhibbard.github.io/xc-cs-college-board/school.html?s=felician)** | Lodi NJ | 13 | CACC | ✅ 2.1% | not reported | not reported | $40,045 | — | — | — | Verify |
| **[Molloy](https://timhibbard.github.io/xc-cs-college-board/school.html?s=molloy)** | Rockville Centre NY | 25 | ECC | ✅ 0.7% | 1088–1290 | 82% | $24,347 | — | — | — | Verify |
| **[Queens College (CUNY)](https://timhibbard.github.io/xc-cs-college-board/school.html?s=queens-college-cuny)** | Queens NY | 10 | ECC | ✅ 10.7% | 1010–1290 | 64% | $4,195 | — | — | — | Verify |
| **[St. Thomas Aquinas](https://timhibbard.github.io/xc-cs-college-board/school.html?s=st-thomas-aquinas)** | Sparkill NY | 20 | CACC | ✅ 3.6% | 880–1085 | 93% | $19,994 | — | — | — | Verify |

**Queens College and the College of Staten Island are the two to email first** — the only CUNYs at
a level where partial athletic aid is possible, both around 10% and 8% CS degrees, both under
$5,600 net. Every D2 school here is unmeasured, so a single ECC/CACC/NE10 championship sweep would
resolve the whole division at once.

### Division 3 (no athletic scholarships)

| School | Location | Mi | Conf. | CS | SAT | Accept | Net cost/yr | XC slot | vs their 7th | 1–7 spread | Tier |
|---|---|---|---|---|---|---|---|---|---|---|---|
| **[NYU](https://timhibbard.github.io/xc-cs-college-board/school.html?s=nyu)** | Manhattan NY | 0 | UAA | ✅ 10.1% | 1480–1560 | 9% | $37,050 | 8 | +35s | 42s | Deep |
| **[Baruch College](https://timhibbard.github.io/xc-cs-college-board/school.html?s=baruch-college)** | Manhattan NY | 5 | CUNYAC | ✅ 9.6% | 1100–1400 | 48% | $3,033 | — | — | — | Verify |
| **[Brooklyn College](https://timhibbard.github.io/xc-cs-college-board/school.html?s=brooklyn-college)** | Brooklyn NY | 10 | CUNYAC | ✅ 10.1% | 950–1340 | 58% | $3,103 | — | — | — | Verify |
| **[CCNY](https://timhibbard.github.io/xc-cs-college-board/school.html?s=ccny)** | Manhattan NY | 6 | CUNYAC | ✅ 4.9% | 1070–1430 | 60% | $3,776 | — | — | — | Verify |
| **[Farmingdale State](https://timhibbard.github.io/xc-cs-college-board/school.html?s=farmingdale-state)** | Farmingdale NY | 35 | Skyline | ✅ 9.4% | 1040–1240 | 63% | $10,867 | — | — | — | Verify |
| **[Hunter College](https://timhibbard.github.io/xc-cs-college-board/school.html?s=hunter-college)** | Manhattan NY | 2 | CUNYAC | ✅ 9.9% | 1100–1420 | 54% | $2,984 | — | — | — | Verify |
| **[John Jay](https://timhibbard.github.io/xc-cs-college-board/school.html?s=john-jay)** | Manhattan NY | 2 | CUNYAC | ✅ 7.4% | 930–1220 | 57% | $3,203 | — | — | — | Verify |
| **[Kean University](https://timhibbard.github.io/xc-cs-college-board/school.html?s=kean-university)** | Union NJ | 20 | NJAC | ✅ 5.6% | 930–1150 | 76% | $12,447 | — | — | — | Verify |
| **[Lehman College](https://timhibbard.github.io/xc-cs-college-board/school.html?s=lehman-college)** | Bronx NY | 11 | CUNYAC | ✅ 8.2% | 850–1150 | 57% | $3,148 | — | — | — | Verify |
| **[Medgar Evers](https://timhibbard.github.io/xc-cs-college-board/school.html?s=medgar-evers)** | Brooklyn NY | 7 | CUNYAC | ✅ 4.8% | 850–1020 | 86% | $5,718 | — | — | — | Verify |
| **[Merchant Marine Acad.](https://timhibbard.github.io/xc-cs-college-board/school.html?s=merchant-marine-acad)** | Kings Point NY | 20 | Skyline | ❌ none | 1110–1300 | 34% | $6,174 | — | — | — | Verify |
| **[Mount Saint Vincent](https://timhibbard.github.io/xc-cs-college-board/school.html?s=mount-saint-vincent)** | Bronx NY | 12 | Skyline | ❌ none | 1011–1178 | 85% | $21,696 | — | — | — | Verify |
| **[Rutgers–Newark](https://timhibbard.github.io/xc-cs-college-board/school.html?s=rutgers-newark)** | Newark NJ | 13 | NJAC | ✅ 6.0% | not reported | 71% | $19,703 | — | — | — | Verify |
| **[Sarah Lawrence](https://timhibbard.github.io/xc-cs-college-board/school.html?s=sarah-lawrence)** | Bronxville NY | 17 | Skyline | ❌ none | 1240–1430 | 62% | $41,437 | — | — | — | Verify |
| **[St. Joseph's Long Island](https://timhibbard.github.io/xc-cs-college-board/school.html?s=st-joseph-s-long-island)** | Patchogue NY | 55 | Skyline | ✅ 4.9% | 1110–1270 | 72% | $19,035 | — | — | — | Verify |
| **[St. Joseph's Univ NY](https://timhibbard.github.io/xc-cs-college-board/school.html?s=st-joseph-s-univ-ny)** | Brooklyn NY | 10 | Skyline | ✅ 4.9% | 1110–1270 | 72% | $19,035 | — | — | — | Verify |
| **[Stevens Institute](https://timhibbard.github.io/xc-cs-college-board/school.html?s=stevens-institute)** | Hoboken NJ | 3 | MAC | ✅ 21.2% | 1380–1505 | 48% | $41,346 | — | — | — | Verify |
| **[SUNY Maritime](https://timhibbard.github.io/xc-cs-college-board/school.html?s=suny-maritime)** | Throggs Neck NY | 13 | Skyline | ❌ none | 1123–1288 | 72% | $22,367 | — | — | — | Verify |
| **[SUNY Old Westbury](https://timhibbard.github.io/xc-cs-college-board/school.html?s=suny-old-westbury)** | Old Westbury NY | 25 | Skyline | ✅ 2.7% | not reported | 84% | $11,282 | — | — | — | Verify |
| **[Yeshiva University](https://timhibbard.github.io/xc-cs-college-board/school.html?s=yeshiva-university)** | Manhattan NY | 5 | Skyline | ✅ 3.3% | 1340–1510 | 56% | $49,965 | — | — | — | Verify |
| **[York College (CUNY)](https://timhibbard.github.io/xc-cs-college-board/school.html?s=york-college-cuny)** | Jamaica NY | 12 | CUNYAC | ✅ 6.3% | 790–960 | 64% | $4,456 | — | — | — | Verify |

> **The two St. Joseph's entries are separate athletic programs** — Brooklyn and Patchogue — sharing
> one federal record, because the Long Island campus has no IPEDS entry of its own. The cost and
> admissions figures are therefore identical by construction, not by coincidence, and they describe
> the university as a whole.
>
> **Four `❌ none` readings.** Merchant Marine (five marine majors and a service obligation) and
> Mount Saint Vincent look like genuine absences. SUNY Maritime and Sarah Lawrence are worth one
> catalog check each before ruling them out, because a CS program housed inside a mathematics
> department reads as zero in the federal completion file — which is exactly what happened at SUNY
> Purchase before the radius change removed it.

### Cross country without men's track

**John Jay, Brooklyn College, SUNY Maritime, Sarah Lawrence** and **SUNY Old Westbury** field
men's cross country with **no men's indoor or outdoor track and field**. For someone who wants both
seasons that is close to disqualifying, and it appears nowhere in the federal data or in a tier —
it is recorded in prose on each school's page. Confirm with the coach before any of them goes on a
shortlist; conference sponsorship tables lag reality by a season. **SUNY Maritime** and the
**Merchant Marine Academy** carry a second constraint: a regiment and summer sea terms, which is a
different life, not a detail.

## 5. Schools removed — he would be a walk-on

| School | Div | Conf. | Location | XC slot | vs their 7th | 1–7 spread | Why removed |
|---|---|---|---|---|---|---|---|
| **[Hofstra](https://timhibbard.github.io/xc-cs-college-board/school.html?s=hofstra)** | D1 | Coastal | Hempstead NY | 8 | +82s | 104s | Cut on cross country data, reversing an earlier recommendation. An outdoor 5000 of 14:32 suggested a clean 40 second gap and this document called it one of the best fits in the metro. They then won the CAA championship with a top seven of 23:02–24:45 — he would be roughly 82 seconds outside the scoring seven, which is walk-on territory by the same rule that cut Furman and Lewis. The team best 5000 was a floor, not a measure of the squad. |
| **[Columbia](https://timhibbard.github.io/xc-cs-college-board/school.html?s=columbia)** | D1 | Ivy | New York NY | — | — | — | 5000 13:43, 3000 8:30, 10K 29:09. Elite — and one of the best CS departments in the country, which is what makes this cut expensive. |
| **[Iona](https://timhibbard.github.io/xc-cs-college-board/school.html?s=iona)** | D1 | MAAC | New Rochelle NY | 8 | +114s | 1s | 5000 14:03, 10K 29:02, 1500 3:38 — a national cross country program, 20 miles out. Cross country confirms it emphatically: at the MAAC championship all seven runners crossed within 0.8 seconds of each other in a deliberate pack finish, 114 seconds ahead of his projection. |
| **[St. John's](https://timhibbard.github.io/xc-cs-college-board/school.html?s=st-john-s)** | D1 | Big East | Queens NY | — | — | — | Nationally strong Big East distance program. |

**Princeton and Rutgers have left this list**, not because the cuts were wrong but because both are
outside the new ring. Columbia and Iona remain the expensive cuts: Columbia has one of the best CS
departments in the country, and Iona is a functioning program 20 miles away. Both return
immediately if a walk-on path ever becomes acceptable.

**NYIT, Montclair State and William Paterson are not on this list either** — there is no men's
cross country program to be removed from. See §2c.

## 6. Tiering

**Target — concentrate effort here.** Fordham · Seton Hall

**Deep — just outside the travel squad.** NYU

**Caution — he arrives at or ahead of their #1.** Manhattan · Saint Peter's · LIU · Fairleigh
Dickinson

**Verify — unmeasured, not borderline (30 schools).** Wagner · NJIT · Stony Brook · all seven D2
schools · twenty of the twenty-one D3 schools

**Cut on results.** Hofstra · Columbia · Iona · St. John's

Thirty-seven schools, **two of them target tier**, and thirty still at Verify. The unresolved
fraction is by far the largest of the three metros — 81% of this board is unmeasured — which is
simultaneously why it ranks third on confidence and why it is the cheapest place to buy
information.

## 7. Ones worth real effort

1. **[Fordham](https://timhibbard.github.io/xc-cs-college-board/school.html?s=fordham)** — still
   the best fit in the metro, and the only one confirmed on cross country rather than inferred:
   **6th man in the Atlantic 10 championship seven**, 12 seconds inside it, against a 95 second
   1-through-7 spread. They train at **Van Cortlandt Park**, the most historic cross country course
   in America, next to campus — which answers the "where do you actually run in New York" question
   better than anyone. The problem is money: **$44,338 net**, the second highest in this metro.
2. **[Seton Hall](https://timhibbard.github.io/xc-cs-college-board/school.html?s=seton-hall)** —
   Big East D1, 18 miles, **$31,446 net**, 73% accept, and a 14:58 team best that leaves him 20–25
   seconds back. Their distance squad is weaker than the conference name suggests, which works in
   his favor. **The only target-tier rating in this metro resting on track marks alone** — their
   conference championship result is the single most valuable lookup left here.
3. **[NJIT](https://timhibbard.github.io/xc-cs-college-board/school.html?s=njit)** — **33.0% of its
   bachelor's degrees are in computer science**, the highest share on the entire 131-school board,
   it is D1, and at **$16,504 net** it is a third the cost of Fordham. If the running fits, this is
   the best D1-plus-CS value in the metro by a wide margin. Everything hinges on roster times that
   do not exist yet.
4. **[Hunter College](https://timhibbard.github.io/xc-cs-college-board/school.html?s=hunter-college)**
   — the new entry to email first. D3 CUNYAC, two miles from Midtown, men's cross country **and**
   track, about 10% of its degrees in CS, **$2,984 net**. Completely unmeasured on running, which is
   the point: at CUNYAC level the odds he is inside their seven are good, and one email settles it.
   Ask for their 2025 CUNYAC 8K results. Then work down the CUNY table in §2b.
5. **[Stony Brook](https://timhibbard.github.io/xc-cs-college-board/school.html?s=stony-brook)** —
   the strongest CS department on Long Island, a full D1 CAA program, **$18,784 net** (but see the
   out-of-state caveat in §8). Sixty miles out and on this board only by the Long Island rule.
6. **[Stevens Institute](https://timhibbard.github.io/xc-cs-college-board/school.html?s=stevens-institute)**
   — if CS leads the decision. The strongest CS on the D3 list at 21.2% of degrees, Hoboken, three
   miles out, no athletic-scholarship pressure, 48% admit. $41,346 net, and unverified on running.

**NYU: too good, not too thin.** Deep rather than Target, and still worth an email. Their 42 second
pack is elite for D3 and he would be 35 seconds off it — not a place he would score as a freshman,
very much a place he would improve. D3 means no scholarship is at stake in aiming high; the 9%
admit rate is the gate.

## 8. NYC-specific considerations

**Cost of living is the dominant variable and the spread is now enormous.** Average net price runs
from **$2,984 at Hunter** to **$49,965 at Yeshiva** — Fordham $44,338, Sarah Lawrence $41,437,
Stevens $41,346, Felician $40,045 and NYU $37,050 at the top; NJIT $16,504 in the middle; ten CUNY
campuses under $6,000. That is a six-figure difference over four years and no partial athletic
scholarship in men's distance running comes close to closing it. Two caveats that pull in opposite
directions: every figure is an average across all students after grants, not a quote for one
applicant; and **the cheapest figures belong to commuter campuses with no dormitories**, where an
out-of-state student's real cost is tuition plus New York rent. Model total cost of attendance, not
tuition.

**Training terrain is a genuine question to raise with coaches.** Ask specifically where the team
does long runs and workouts. Good answers: Van Cortlandt Park (Fordham, Manhattan), Central Park,
Prospect Park, the Rockefeller State Park carriage trails, Bear Mountain for hills, Sunken Meadow
and Bethpage on Long Island. A bad answer is a hesitation, or "we do a lot on the track." At a
commuter school, ask a harder version of the same question: where does the team actually meet, and
how many of them get there. Each school's detail page maps every meet it attended last year, which
shows how far and how often a program really travels.

**Two states, two sets of in-state tuition, neither of them his.** With Connecticut out of range
this metro is New York and New Jersey only. The New Jersey publics (Kean, NJIT, Rutgers–Newark) and
the New York publics (the ten CUNYs, Farmingdale, Old Westbury, SUNY Maritime, Stony Brook) will
all charge out-of-state rates to a South Carolina applicant. The gap is small at CUNY — roughly
$7,400 in-state against $15,300 out — and large at Stony Brook, where it is $10,931 against
$32,741. Check it school by school before ranking any of them on price.

## 9. To-dos, priority order

1. **Pull the CUNYAC championship result.** One document resolves eight of the ten CUNY colleges at
   once, and they are the cheapest schools on the entire board. This is the highest
   information-per-email action available in any metro.
2. **Get cross country results for Seton Hall, NJIT and Stevens.** Seton Hall is the target-tier
   rating resting on track marks; NJIT and Stevens are the two best CS options here.
3. **Pull the remaining D2 and D3 conference championship results** — ECC, NE10, CACC, NJAC, Skyline,
   East Coast, MAC and America East. That sweep plus the CUNYAC would resolve nearly all thirty
   Verify-tier schools.
4. **Confirm the five no-men's-track findings** with each coach — John Jay, Brooklyn College, SUNY
   Maritime, Sarah Lawrence, SUNY Old Westbury. A conference sponsorship table can be a season out
   of date in either direction.
5. **Get a second race for Wagner.** One result with six finishers and a 360 second spread cannot be
   trusted in either direction.
6. **Calibrate the northeastern courses.** The NEC, UAA, Big East, MAAC and CAA courses in this
   dataset have no cross-checking team on a calibrated course, so their times cannot yet be put on
   a common scale.
7. Confirm the computer science situation at **SUNY Maritime and Sarah Lawrence**, where the federal
   file shows no bachelor's CS program — the SUNY Purchase case says check the catalog before ruling
   it out.
8. Verify SAT figures for **Fairleigh Dickinson, Saint Peter's, Felician, Rutgers–Newark and SUNY
   Old Westbury**, which report no SAT range in the federal file, and Felician's missing admit rate.
9. Get real out-of-state cost estimates, not net-price averages, for the ten CUNYs — including
   housing, since most have no dorms.
10. Ask each coach the CS-lab-conflict question (see `greenville-sc.md` §9).

**Resolved and off this list:** whether NYIT still sponsors men's cross country (§2c — it sponsors
no sports at all), Fairfield's disagreeing races and Wagner's tier (Fairfield is now out of range),
and SUNY Purchase's CS reading (also out of range, but the lesson carries).
