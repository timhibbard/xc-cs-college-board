...
| Script | What it does |
| --- | --- |
| `render-check.js` | Renders three metro pages, `north-carolina.html` and every school page with jsdom, and asserts row counts derived from `assets/data.js` plus the totals README.md and methodology.html publish in prose — the board totals, the coach-block count, the Instagram coverage, the 1500 fields and the schedule totals (meets, venues, appearances and the precision split, in all three files that publish them). The North Carolina block checks that page's grouping both ways — every NC row is in exactly one `NC_REGIONS` group and every name a group lists is a row that exists — because a misspelled name there renders a short table and no error, and it holds the hand-filled count on that page's `index.html` card against the data. It also holds every venue inside a bounding box for the state its own string names — a wrong state is a wrong pin however sure the geocoder was, and that check found five. The 1500 block asserts the promise #5 closed on (every row with a `b1500` has a `T1500` to drop his projection into, keyed by **slug** — the one thing issue #5 itself got wrong) rather than only a count, and holds `dslot` against `d15` wherever the seven-man cap does not hide a faster man. Needs `jsdom`: `npm install --no-save jsdom`, or point `NODE_PATH` at an install that has it. |
| `page-check.js` | Renders all 14 site pages (it discovers them, so a new page needs no change here), reporting uncaught JS errors, dead relative links and broken in-page anchors. This is what caught the `washington.html` failure, where a missing `id="k-lead"` threw and took the master table, the map and three off-board tables down with it. |
...
edit assets/data.js
  ...
  → node tools/page-check.js          # all 14 pages render, no dead links
...
| `remeasure_mi.py` | `.work/board.json` | `.work/mi.json` — every row's `mi` re-routed on OSRM from **its own assigned** metro centre, to one decimal. `stage1.py` picked the nearest of 20 centres and stopped at the first inside 20 miles, so part of the column was a different question from the rest, and six of thirty sampled rows disagreed with the router. Nothing is dropped: a row that now measures past its metro's radius is printed loudly and left in place. Exports `drive_mi()` |
| `apply_mi.py` | `.work/mi.json` | rewrites every `mi:` in `assets/data.js`, keyed on the name in the same line. A row absent from `mi.json` is left alone |
| `nc_seven_mi.py` | `.work/board.json`, `.work/HD2023.csv` | `.work/nc_seven.json` — the seven NC schools `north-carolina.html` names beyond the ring, measured from the same origin on the same router as the column, so that page's comparability claim survives a re-measure. Coordinates from IPEDS HD, unit IDs explicit |
| `setting_locale.py` | `.work/HD2023.csv`, both 2023 Census Gazetteer files (`place` **and** `cousubs`) unzipped into `.work/` | `.work/setting1.json` — federal `LOCALE` code, street address and the row's own town with its distance and land area. The subdivision file is not optional and it **fills only, never overrides**: New England towns, NJ/PA townships and NYC boroughs are in no place file, and letting the larger area win moved Appalachian State from Boone town to Boone township, 78 miles out |
| `setting_walkscore.py` | `.work/setting1.json` | `.work/walkscore.json` — Walk, Transit and Bike Score per row with the page they came from, plus the named rail and bus lines with distances (`--lines`). Nothing is stored unless the page's own map coordinate lands near the campus coordinate the board holds |
| `setting_overpass.py` | `.work/setting1.json` | `.work/overpass.json` — the training axis: nearest park with its acreage, soft-surface and off-road mileage, crossings per mile, campus acreage, nearest track. ~110s per row; **in progress, see #24** |
| `apply_setting.py` | `.work/setting1.json`, `.work/walkscore.json` | writes `SETTING` into `assets/data.js` (keyed by **name**, so the twelve rows with no page still get one) and `SETLINES` into `assets/detail.js` (keyed by **slug**, the full named rail and bus lists). Replaces both blocks in place, so re-running is safe and it is the only thing that writes them. Refuses to run if any row has no Walk Score or never had its transit lines read, because a partial splice puts the two files out of step with each other — `render-check.js` holds `railN`/`busN` against the lists for exactly that reason |
